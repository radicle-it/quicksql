...
// ── Module-level ddl reference (set by recognize, used by DdlNode methods) ───

let ddl: DdlContext | null = null;
...
function pkTypeModifier(objName: string, naming: Naming = DEFAULT_NAMING): string {
    if (ddl!.optionEQvalue('pk', 'identityDataType')) return IDENTITY_MODIFIER_LOWER;
...
export class DdlNode {
    ...
    constructor(lineNo: number, inputLine: string, parent: DdlNode | null) {
        this.line    = lineNo;
        this.parent  = parent;
        this.children = [];
    ...
    maxChildNameLen(): number {
        ...
        if (this.hasAuditCols()) {
            for (const key of ['createdcol', 'createdbycol', 'updatedcol', 'updatedbycol']) {
                const len = String(ddl!.getOptionValue(key) ?? '').length;
        ...
        const cols = ddl!.additionalColumns();
    ...
    auditSysDateFn(): string {
        const auditDateType = String(ddl!.getOptionValue('auditdate') || ddl!.getOptionValue('Date Data Type') || '');
    ...
    sugarcoatName(from: number, to: number): string {
        ...
        for (let i = from; i < to; i++) {
            ...
            if (this.src[i].type !== 'constant.numeric' && value !== canonicalObjectName(qVal)) {
                ...
                const hasPrefix = ddl !== null && 0 < (String(ddl.getOptionValue('prefix') ?? '')).length;
    ...
    _inferTypeFull(): InferTypeResult {
        ...
        let len: number = (colName.endsWith('_name') || colName.startsWith('name') || colName.startsWith('email'))
            ? (ddl!.getOptionValue('namelen') as number) || 255
            : 4000;
        ...
        let ret = `varchar2(${len}${ddl!.semantics()})`;
        ...
        const dbVer = ddl!.getOptionValue('db') as string | null;
        if (booleanCheck !== '' && (
            ddl!.getOptionValue('boolean') === 'native'
            || (ddl!.getOptionValue('boolean') !== 'yn' && dbVer && dbVer.length > 0 && 23 <= (getMajorVersion(dbVer) ?? 0))
        )) {
            booleanCheck = '';
            ret = 'boolean';
        }
        ...
        if (_nameImpliesDate(colName, src, datePos))
            ret = String(ddl!.getOptionValue('Date Data Type') ?? '').toLowerCase();
    ...
    inferType(): string {
        ...
        if (this.isOption('fk') || 0 < this.indexOf('reference', true)) {
            ...
            const refNode = ddl!.find(parentRef!);
    ...
    refId(): string | null {
        ...
        if (0 < pos) {
            ...
            if (ddl!.find(ref) !== null) return ref;
    ...
    cardinality(): number {
        ...
        if (start) {
            ...
            const limit = ddl!.getOptionValue('datalimit') as number;
    ...
    OUTER: for (let i = 0; i < src.length; i++) {
        ...
        if (t.value === '\n') {
            if (poundDirective === null) {
                ...
                let node = new DdlNode(t.line - 1, line, null);
                let matched = false;
        ...
        if (poundDirective !== null) {
            ...
            for (const j in src1) {
                ...
                if (flattened !== null) {
                    ...
                    try { ddl.data = JSON.parse(jsonStr); poundDirective = null; line = ''; continue OUTER; }
                    catch (_e) { /* keep going */ }
                }
        ...
        for (let fk in node.fks) {
            ...
            if (0 < fk.indexOf(',')) {
                ...
                for (let i = 0; i < chunks.length; i++) {
                    ...
                    ret += tab + col + colPad + pChild.inferType() + ',\n';
                }
                continue;
            }
            let type = 'number';
...
`;
    ...
    _resolveFkSampleValues(_node: DdlNode, fk: string, ref: string, elem: unknown, pkValue: unknown, objName: string): { type: string; values: unknown[] } {
        ...
        let values: unknown[] = [];
        let type = 'INTEGER';
        for (let k = 1; k <= refNode.cardinality(); k++) values.push(k);
        if (elem !== null && elem !== undefined) {
