    ...
    OracleDDLGenerator = class OracleDDLGenerator {
        ...
        _buildColumnConstraints(node, ret, booleanCheck, isNativeBoolean, parent_child) {
            ...
            if( node.isOption('unique') || node.isOption('uk') ) {
                ret += '\n';  
                ret += tab +  tab+' '.repeat(node.parent.maxChildNameLen()) +'constraint '+concatNames(this._ddl.objPrefix(),parent_child,'_unq')+' unique';
            } 
            var optQuote = '\'';
            if(  ret.startsWith('integer') || ret.startsWith('number') || ret.startsWith('date')  ) 
                optQuote = '';
            ...
            if( node.isOption('pk') ) {
                let typeModifier = ret.startsWith('number')
                    ? ' ' + pkTypeModifier(this._ddl.objPrefix() + node.node.parent.parseName())
                    : ' not null';
                ret += typeModifier + '\n';
                ret += tab + tab + ' '.repeat(node.parent.maxChildNameLen()) + 'constraint ' + concatNames(this._ddl.objPrefix(),parent_child,'_pk')+' primary key';
            }
        ...
        _genTableHeader(node, objName, immutableKeyword, idColName) {
            ...
            if( idColName != null && !node.isOption('pk') ) {
                ...
                ret += tab +  tab+' '.repeat(node.maxChildNameLen()) +'constraint '+concatNames(obj_col,'_pk')+' primary key,\n';
            } else {
        ...
        _genFkColumns(node, objName) {
            ...
            for( let fk in node.fks ) {
                ...
                if( refNode.line < node.line || refNode.isMany2One() ) {
                    ret += tab + tab+' '.repeat(node.maxChildNameLen()) + 'constraint '+objName+'_'+fk+'_fk\n';
                    let onDelete = '';
        ...
        _genRowKeyColumn(node, objName) {
            ...
            ret += tab +  tab+' '.repeat(node.maxChildNameLen()) +'constraint '+objName+'_row_key_unq unique not null,\n';
            return ret;
        }
